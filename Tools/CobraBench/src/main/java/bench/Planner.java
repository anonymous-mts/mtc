...
import bench.generalBench.GeneralBench;
import bench.miniBench.MiniBench;
...
public class Planner {
	...
	private Planner() {
		...
		switch (Config.get().BENCH_TYPE) {
			...
			case WOROANOMALY:
				...
				break;
			case MINI:
				this.bench = new MiniBench(kvi);
				break;
			case GENEARAL:
				this.bench = new GeneralBench(kvi);
				break;
			default:
				assert false;
				break;
	...
	private void waitForRunners() {
		...
		while (!threadPool1.isEmpty()) {
			...
			// System.out.println("progress%=" + df.format((((double) complete_txn) /
			// total_txn) * 100) + "%"
	...
	public String getResult() {
		...
		for (int i = 0; i < tags.length; i++) {
			...
			sb.append("  " + tag + ": " + df.format(latency / 1000000) + "ms  [" + time / 1000000 + "ms/"
					+ df.format(counter / 1000.0) + "k]\n");
		}
