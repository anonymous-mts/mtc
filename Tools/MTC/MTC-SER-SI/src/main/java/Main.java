...
import history.loaders.CleanTextHistoryLoader;
...
@Command(name = "verifier", mixinStandardHelpOptions = true, version = "verifier 0.0.1", subcommands = { Audit.class,
        Convert.class, Stat.class, Dump.class, AuditSER.class, AuditMini.class })
...
@Command(name = "auditMini", mixinStandardHelpOptions = true, description = "Verify a mini history against SI")
class AuditMini implements Callable<Integer> {
    @Option(names = { "--log" }, description = "use log" )
    ...
    public Integer call() throws Exception {
        ...
        var pass = true;
        var miniVerifier = new MiniVerifier<>(loader, log);
        pass = miniVerifier.audit();
...
@Command(name = "auditSER", mixinStandardHelpOptions = true, description = "Verify a history against SER")
class AuditSER implements Callable<Integer> {
    @Option(names = { "-t", "--type" }, description = "history type: ${COMPLETION-CANDIDATES}")
    private final HistoryType type = HistoryType.COBRA;
    ...
    public Integer call() throws Exception {
        ...
        var pass = true;
        var serVerifier = new SERVerifier<>(loader);
        pass = serVerifier.audit();
        profiler.endTick("ENTIRE_EXPERIMENT");
...
class Utils {
    static HistoryLoader<?, ?> getLoader(HistoryType type, Path path) {
        switch (type) {
            ...
            case CLEANTEXT:
                return new CleanTextHistoryLoader(path);
            case ELLE:
...
enum HistoryType {
    COBRA, DBCOP, TEXT, ELLE, CLEANTEXT
}
