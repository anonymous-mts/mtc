...
for max_txn_length in max_txn_length_set:
  ...
  for _ in range(test_count):
    ...
    log_path = os.path.join(root_path, 'store', f'stolon-{workload}-S-S-')
    check_time = 0
    abort_rate = 0
    for log_dir in os.listdir(log_path):
      ...
      log_path = os.path.join(root_path, 'store', f'stolon-mini-S-S-/latest/history.edn')
      output_path = os.path.join(root_path, 'store', f'stolon-mini-S-S-/latest/1.txt')
      jar_path = os.path.join(root_path, 'PolySI-1.0.0-SNAPSHOT.jar')
      check_path = 'store/stolon-mini-S-S-/latest/1.txt'
