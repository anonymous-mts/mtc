  ...
  (setup! [_ test]
    (dotimes [i (:table-count test default-table-count)]
      ...
      (with-retry [conn  conn
                   tries 10]
        (j/execute! conn [(str "drop table " (table-name i))])
        (j/execute! conn
...
(defn real-ops-value-and-keys
  ...
  [op-list [key1 key2 key3]]
  (let [key-map {:key1 key1, :key2 key2, :key3 key3}]
...
(defn precede-interleave
  "保证r1在w1前面，r2在w2前面，r3在w3前面，同时合并r-list和w-list"
  [r-list w-list]
  (let [m {:w1 :r1, :w2 :r2, :w3 :r3}]
    (cond
      (empty? r-list) 
      w-list
      (empty? w-list) 
      r-list
      (some #(= ((first w-list) m) %) r-list) 
      (cons (first r-list) (precede-interleave (rest r-list) w-list))
      :else 
      (cons (first w-list) (precede-interleave r-list (rest w-list))))))

(defn r1w1r2w2r3w3
  "保证读操作在对应写操作前的三个组合"
  [key-list]
  (real-ops (precede-interleave (shuffle [:r1 :r2 :r3]) (shuffle [:w1 :w2 :w3])) key-list))
...
(defn rand-key
  ...
  (case key-dist
    :uniform     (let [_ (swap! index-counter inc)]
                   (nth active-keys (mod @index-counter (count active-keys))))
...
(defn diff-key-txn-and-next-state
  ...
  (let [max-writes-per-key   (:max-writes-per-key  opts 32)
        ...
        key2 (rand-key key-dist key-dist-base
                       key-dist-scale active-keys)
        key3 (rand-key key-dist key-dist-base
                       key-dist-scale active-keys)
      ...
      (if (< max-writes-per-key v1)
        ...
        (if (< max-writes-per-key v2)
          (let [state' (update state :active-keys
                               fresh-key key2)]
            (recur opts state'))
          (if (< max-writes-per-key v3)
            (let [state' (update state :active-keys
                                 fresh-key key3)]
              (recur opts state'))
            (let [txn (f [key1 key2 key3])
                  ...
                  state'' (if writes-key2?
                            ...
                            state')
                  writes-key3? (writes? txn key3)
                  state'' (if writes-key3?
                            (assoc state' key3 (inc v3))
                            state')]
              [txn state''])))
          ))))
...
(defn mini-txns
  ...
  ([opts]
   (let [key-dist  (:key-dist  opts :uniform)
